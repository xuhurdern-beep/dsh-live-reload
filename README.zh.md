...
> 收场。这个插件就是通用解法:设置页一个按钮热应用整个组合,刷新成功后总是自动重载页面,
> 让新的*客户端* bundle 落地到浏览器(宿主进程本身永不退出)。
...
- **结果报告** —— 按钮展示新增/移除/更新行与激活错误;刷新成功后**总是自动重载页面**
  (不做"是否出现新客户端 bundle"的差异检测——重载会按最新 boot manifest 拉取,任何新插件
  的客户端 bundle 都必然被加载,市场热安装因此每次都能直接生效)。失败详情会透传**完整错误链**
  (含 `AggregateError.errors`),类似 `failed to rollback loader entry ...: ` 这种被包装的
  报错不会再隐藏真正原因。
...
- **插件包热更新(0.2.0)** —— 每个 bundle 都会在磁盘上做指纹快照(启动时播种);当已挂载
  ...
  撞名(loader 会先撤销旧 fiber 的注册再启动新行)。刷新还会**驱逐该包在 Node ESM loadCache
  里的全部缓存模块**(入口文件 + 相对依赖——仅给入口加 `?dshr=` 覆盖不到 `./dep.js`),所以
  改动发生在非入口文件(比如 `lib/*.js` 新增导出)的升级同样能加载到新代码。
- **碰撞自愈(0.2.0)** —— 若刷新仍撞上 `tool "X" is already registered`,插件会识别肇事
  包、强制破除其模块缓存并重试一次——无需重启即可自愈(结果里出现 `selfHealed`)。该机制
  同样覆盖市场的**热挂载残留**:插件被热安装(`mkt-*` 行,市场运行时子树)且 bundle 层随后
  拥有同一包时,刷新会先移除冲突的 `mkt-*` 行(撤销其注册)再重新应用 bundle 层行。
...
2. 点击 **一键刷新插件并重载页面 / Refresh Plugins & Reload**。
3. 成功后页面会**短暂停留并自动重载**——新的客户端 bundle(比如刚装插件的 UI)落地到
   浏览器,宿主进程与会话全程不退出;失败时错误面板保持可读,不会重载。
...
设置页按钮 ──POST──▶ /dsh-live-reload/refresh
        ...
        loader 协调:挂载新行 · 配置更新变更行 · 移除被删行
                          │
                          ▼
        审计(每个启用行都有活动 fiber)+ diff 报告
                          │
                          ▼
        成功 → 客户端自动重载页面(宿主进程不退出)
```
